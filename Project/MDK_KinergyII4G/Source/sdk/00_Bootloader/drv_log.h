...
#include "bsp_log_manager.h"  // 直接使用当前工程的日志系统
...
#define macro_log_sector log_sector_printf//((t_log_sector)BootUARTFunc(log_sector_printf))
    ...
    #define log_error(fmt, ...)  LOG_PRINTF(LOG_MODULE_SYSTEM, LOG_LEVEL_ERROR, fmt, ##__VA_ARGS__)
#else
    #define log_error(...)
...
#define log_warning(fmt, ...) LOG_PRINTF(LOG_MODULE_SYSTEM, LOG_LEVEL_WARN, fmt, ##__VA_ARGS__)
#else
#define log_warning(...)
#endif

#if(LOG_LEV_VAL >=  LOG_LEV_INFO )
    #define log_info(fmt, ...)  LOG_PRINTF(LOG_MODULE_SYSTEM, LOG_LEVEL_INFO, fmt, ##__VA_ARGS__)
...
#define log_debug(fmt, ...)      LOG_PRINTF(LOG_MODULE_SYSTEM, LOG_LEVEL_DEBUG, fmt, ##__VA_ARGS__)
#if (LOG_RAM_OFFSET_EN)
#define log_line()
#else
#define log_line() LOG_PRINTF(LOG_MODULE_SYSTEM, LOG_LEVEL_DEBUG, "%s[L-%d]\r\n",__func__,__LINE__)
#endif
#else
#define log_debug(...)
#define log_line()
