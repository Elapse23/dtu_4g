...
typedef enum
{
    ...
    //sim
    sim_open_connection_success,
    sim_open_connection_fail,    //all cmd err
    sim_connect_fail,            //only connect err
    sim_close_connection_success,
    sim_close_connection_fail,
    sim_connection_closed,
    sim_connection_auto_connect_success,
    sim_get_info_fail,
    sim_get_info_success,
...
{
    uint8_t b_channel_index;
	lte_module_ap_info *pst_data_info;
}lte_module_get_ap_info_connection;
typedef enum
{
    //Initial
...
typedef struct
{
    lte_module_operate_type enum_operate_type;
    lte_module_driver_operate_result_notify_callback pfunc_operate_result_notify;
    union
    {
        //ap
        lte_module_open_ap_connection st_open_ap_connection;
        lte_module_get_ap_info_connection st_get_ap_info_connection;
        //tcp
...
}lte_module_driver_config_info_general;
...
typedef struct
{
    genaral_communicate_data_info *pst_data;
}lte_module_driver_reponse_info_general;

#define lte_module_ip_max_len            						4
#define lte_module_port_max_len          						2
typedef struct
{
    uint8_t ba_ip[lte_module_ip_max_len];
    union port
    {
        uint16_t w_port;
        uint8_t ba_port[lte_module_port_max_len];
    }un_port;
}lte_module_ip_port;
...
typedef struct
{
    ...
    lte_module_driver_config_info_general *pst_config_info;
}lte_module_driver_process_contral;
