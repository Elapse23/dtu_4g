...
*********************************************************/
//variables - 使用宏定义将变量映射到指定地址
// 这些定义已移动到 BootLoader.h 中，避免重复定义
...
__attribute__((section(".firmware_info"))) const INT8U bFrimwareVerStr[3] =
{
    (INT8U)cbAppVersion1,
...
__attribute__((section(".firmware_info"))) const INT8U bBootFirmwareStr[2] =
{
    (INT8U)cbBootVersion1,
    (INT8U)cbBootVersion2,
};

__attribute__((section(".firmware_info"))) const INT8U bFirmwareTypeStr[4] =
{
...
__attribute__((section(".firmware_info"))) const INT8U bFirmwareAppStartAddr[4] =
{
...
__attribute__((section(".firmware_info"))) const INT8U bFirmwareBootEndAddr[4] =
{
...
// 直接使用标准C语法，不使用IAR特定的内存定位
const INT8U bFirmwareBootStartAddr[4] =
{
...
const INT32U dwBootExtTbl[cbMaxExtTable] =
{
    0,
...
const INT8U bBootRodataEnd[2] = {0xFF,0xFF};
const INT16U wCRCTableAbs[16] =
{
    0x0000,
    0xCC01,
    0xD801,
...
const INT16U wBootRxDelay_500us[cbSetBandRateMax] =
{
...
const INT32U cwBootBaudRateTab[cbSetBandRateMax] =
{
...
const INT8U bBootRodataStart[cbBootReverseDataLen] =
{
    0xFF,
};
#else
...
static void sICPProcess(void)
{
    // 添加调试标记，确认进入了sICPProcess
    volatile uint32_t icp_process_marker = 0x1CCC1CCC;
    (void)icp_process_marker;
...
//    INT16U wDelayCnt = 0;
    ...
    // 添加调试标记
    volatile uint32_t debug_boot_mode = boot_mode;
    volatile uint32_t debug_cbUpdateFinish = cbUpdateFinish;
    (void)debug_boot_mode; (void)debug_cbUpdateFinish;
    
    sClearReverseRam();
    // 强制进入应用程序跳转分支
    // if(cbUpdateFinish == boot_mode){
    if(true){  // 强制条件成立
        // 临时修改：总是认为应用程序有效，直接跳转
...
// 缺失的函数实现
// 全局变量：应用程序有效性标记
volatile uint32_t app_valid_marker = 0x88888889;  // cdwAPPMark的值

void sAppOkMark(void)
{
    // 应用程序标记地址
    // 这个函数的地址被用作应用程序有效性标记的存储位置
    // 我们使用全局变量来存储标记值
    app_valid_marker = 0x88888889;  // 确保标记值正确
}

void sDoIarcmain(void)
{
    // 跳转到应用程序主函数
    // 设置多个调试标记来跟踪执行流程
    volatile uint32_t bootloader_jump_marker = 0xB007AAAA;
    
    // 标记开始准备跳转
    volatile uint32_t prepare_jump_marker = 0xBBBBBBBB;
    
    // 尝试调用main函数
    volatile uint32_t call_main_marker = 0xCCCCCCCC;
    
    // 直接调用main函数而不是重启
    extern int main(void);
    
    // 更新标记表示即将调用main
    call_main_marker = 0xCCCCCCCD;
    
    main();
    
    // 如果main返回，设置返回标记
    volatile uint32_t main_returned_marker = 0xDDDDDDDD;
    (void)bootloader_jump_marker; (void)prepare_jump_marker; 
    (void)call_main_marker; (void)main_returned_marker;
